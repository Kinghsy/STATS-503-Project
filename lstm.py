...
hidden_dim = 400
hidden_layer_num = 2
sentence_max_length = 800
title_len_maximum = 80
feat_num = 8
lr = 0.01
momen = 0.9
...
class rnn_net(nn.Module):
    def __init__(self, sent_len, output_dim,
                 init_weight, embed_index, embed_dim, feat_num=feat_num):
        super().__init__()
        ...
        # rnn layer
        self.rnn_layer = torch.nn.LSTM(input_size=embed_dim,
                                       hidden_size=feat_num,
                                       bidirectional=True)

        # nn layer
        head_dim = feat_num * sent_len * 2
        self.fc1 = torch.nn.Linear(head_dim, hidden_dim)
...
if __name__ == "__main__":
    ...
    net = rnn_net(sentence_max_length, ttls[0],
